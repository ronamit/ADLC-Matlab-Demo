function [idx,centers] = KMeansClustering(X, k, visualize2D, centers)
...
% centers - Centroids (Cluster centres)
    ...
    if ~exist('centers', 'var')
        ...
        %                                                                     %
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        centersInd = randperm(m,k); % choose randomly k points
    ...
    while true        
        ...
        distCToX = zeros(m,k);
        for cInd=1:k
           dist = bsxfun(@minus,X,centers(cInd,:));
           distCToX(:,cInd) = sum((dist').^2);   
        end
        [~,idx]= min(distCToX,[],2);
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        for cInd=1:k
            isBelongToC = (idx==cInd);
            centers(cInd,:) = mean(X(isBelongToC,:));
        end
        ...
        if n == 2 && visualize2D
            VisualizeClusters2D_ours(X, idx, centers, figHandle);
