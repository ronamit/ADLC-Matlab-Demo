...
m = size(X, 1);
assert(m <= 1e4); % prevent memory overflow
...
if n == 2 && visualize2D
    VisualizeClusters2D_ours(X, idx, centroids, figHandle);
    pause;
end
...
while num_clusters > k
    ...
    %                                                                     %
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    [~, minLinearInd] = min(dists(:));
    [i, j] = ind2sub(size(dists), minLinearInd);
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %                                                                     %
    %                            END YOUR CODE                            %
    %                                                                     %
    ...
    %                                                                     %
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    centroids(i,:) = (centroids(j,:)*cluster_sizes(j) + centroids(i,:)*cluster_sizes(i))/(cluster_sizes(i)+cluster_sizes(j));
    ...
    %                                                                     %
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    dists(:, i) = sqrt(sum(bsxfun(@minus ,centroids, centroids(i,:)).^2,2));
    dists(i, :) = dists(:, i);
    dists(i, i) = inf;
    dists(j, :) = inf;
    dists(:, j) = inf;
...
validInds = ~isinf(centroids(:, 1));
centroids = centroids(validInds, :);
end